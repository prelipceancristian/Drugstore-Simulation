...
class GenericFileRepository:
    ...
    def update_binary_search(self, entity):
        """
        Updates an entity.
        :param entity: the entity to update
        :return: -
        ...
        for id in self.__storage:
            list_of_ids.append(id)

        list_of_ids = sorted(list_of_ids, key=lambda c:c)

        id_entity = entity.id_entity
        # if id_entity not in self.__storage:
        if binary_search(id_entity, list_of_ids) is False:
            raise RepositoryError('There is no entity with that id!')
        self.__storage[id_entity] = entity
        self.__save_to_file()
